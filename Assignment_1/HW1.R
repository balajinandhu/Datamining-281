...
ggplot(data=censusData, aes(x=age, y=hrs_per_week)) +geom_point(shape=1, alpha=0.08,position=position_jitter(width=0.1,height=0.1))
hexbinplot(hrs_per_week ~ age , censusData, aspect=1.5,style = "centroids")

thrs<-censusData$hrs_per_week
tage<-censusData$age
cor(tage,thrs)